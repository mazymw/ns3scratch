...
double LteGymEnv::ScaleGlobalSinr(double sinrDb) {
    if (sinrDb < 0.0)
        return std::max(-2.0, 2.0 * (sinrDb / 10.0));  // Clamp at -2.0
    else if (sinrDb <= 10.0)
        return sinrDb / 10.0;
    else
        return 1.0;
...
float LteGymEnv::GetReward()
{
    ...
    const double w_power = 0.1 / numSbs;   // per-SBS normalized weight
    const double w_sinr = 4.0;
    const double w_efficiency = 0.01;
    const double switching_penalty_weight = 0.000000001 / numSbs;
    ...
    for (const auto& [sbsId, model] : m_energyModels)
    {
        ...
        if (currentState == SmallCellEnergyModel::ACTIVE)
            numActiveSbs++;
    ...
    double sinrReward = (activeUeCount > 0)
        ? scaledGlobalSinr * std::sqrt(static_cast<double>(activeUeCount) / totalUeCount)
        : 0.0;

    // === Energy Efficiency Component ===
    double efficiencyReward = (totalPower > 0.0)
        ? static_cast<double>(activeUeCount) / totalPower
        : 0.0;

    totalReward += w_sinr * sinrReward;
    totalReward += w_efficiency * efficiencyReward;
    ...
    std::cout << "[Reward Breakdown] SINR=" << (w_sinr * sinrReward)
        //   << ", Power=" << (w_power * scaledPower * numSbs)
          << ", EE=" << (w_efficiency * efficiencyReward)
          << std::endl;

    std::cout << "[Global SINR] " << m_globalAvgSINR << " dB (scaled: " << scaledGlobalSinr << ")\n";
    std::cout << "[UEs] Active: " << activeUeCount << " / " << totalUeCount << std::endl;
    std::cout << "[Efficiency] UEs/W = " << efficiencyReward << std::endl;

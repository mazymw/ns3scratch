...
public:
      ...
      m_transitionEndTime(Seconds(0))
  {
    ...
    m_activationDelay[SM1] = Seconds(0.0355);
    m_activationDelay[SM2] = Seconds(0.5);
    m_activationDelay[SM3] = Seconds(5);
  ...
  void SetState(SmallCellState state)
  {
      ...
      // Prevent switching out of ACTIVE before 0.5s after transition to ACTIVE
      const double minActiveDuration = 1.5; // seconds
      if (m_currentState == ACTIVE && state != ACTIVE) {
          ...
          if (timeInActive.GetSeconds() < minActiveDuration) {
              ...
              // Schedule this same SetState call after the remaining time
              Simulator::Schedule(Seconds(remaining), &SmallCellEnergyModel::SetState, this, state);
              return;
          }
      }
      ...
      if (m_currentState != state) {
          ...
          if (state == ACTIVE && m_currentState != ACTIVE) {
              ...
              m_transitioning = false;
              m_transitionEndTime = Seconds(0);
              // Only update active entered time if now entering ACTIVE with NO delay
              if (state == ACTIVE) {

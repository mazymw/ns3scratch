...
//     std::cout << "\n[DEBUG] Calculating SINR for UE NodeId=" << ueNode->GetId() 
//               << ", Candidate eNB NodeId=" << candidateEnbNode->GetId() << std::endl;
//     std::cout << "    [DEBUG] UE position: " << ueMob->GetPosition() 
//               << ", Candidate eNB position: " << candidateEnbMob->GetPosition() << std::endl;
//     std::cout << "    [DEBUG] Distance between UE and candidate eNB: " << distance << " m" << std::endl;
//     std::cout << "    [DEBUG] Candidate eNB TxPower: " << candidateTxPowerDbm << " dBm" << std::endl;

//     // Signal: received from the candidate eNB
//     double rxSignalDbm = CalculateRxPowerDbm(candidateTxPowerDbm, distance, 2600.0, pathLossExponent);
//     double rxSignalWatt = dBmToWatt(rxSignalDbm);
//     // std::cout << "    [DEBUG] Rx Power from candidate eNB: " << rxSignalDbm << " dBm ("
//     //           << rxSignalWatt << " W)" << std::endl;

//     // Interference: sum from all other eNBs
...
//         if (enbNodeId == candidateEnbNode->GetId()) continue; // skip candidate

//         Ptr<MobilityModel> otherEnbMob = enbDev->GetNode()->GetObject<MobilityModel>();
//         double otherDistance = ueMob->GetDistanceFrom(otherEnbMob);

//         Ptr<LteEnbPhy> enbPhy = enbDev->GetPhy();
//         double otherTxPowerDbm = enbPhy ? enbPhy->GetTxPower() : 44.77;
//         double rxInterfDbm = CalculateRxPowerDbm(otherTxPowerDbm, otherDistance, 2600.0, pathLossExponent);
//         double rxInterfWatt = dBmToWatt(rxInterfDbm);

//         interferenceWatt += rxInterfWatt;

//         std::cout << "    [DEBUG] Interfering eNB NodeId=" << enbNodeId
//                   << " | TxPower=" << otherTxPowerDbm << " dBm"
//                   << " | Distance=" << otherDistance << " m"
//                   << " | Rx Power at UE=" << rxInterfDbm << " dBm ("
//                   << rxInterfWatt << " W)" << std::endl;
//     }
//     std::cout << "    [DEBUG] Total Interference Power at UE: " << interferenceWatt << " W" << std::endl;

//     // Noise power (thermal): N = kTB, T=290K, B=20MHz (LTE typical)
//     double noiseWatt = 1.38e-23 * 290 * 20e6;
//     std::cout << "    [DEBUG] Noise Power: " << noiseWatt << " W" << std::endl;

//     // SINR = signal / (interference + noise)
//     double denominator = interferenceWatt + noiseWatt;
//     if (denominator <= 0.0) denominator = 1e-12; // Avoid divide by zero
//     double sinr = rxSignalWatt / denominator;
//     double sinrDb = 10.0 * std::log10(sinr);

//     std::cout << "    [DEBUG] SINR (linear): " << sinr << std::endl;
//     std::cout << "    [DEBUG] SINR (dB): " << sinrDb << std::endl;
...
void CustomSinrHandover(
    ...
    for (uint32_t i = 0; i < ueDevs.GetN(); ++i) {
        ...
        if (bestEnbNodeId != servingEnbNodeId && (bestSinrDb - servingSinrDb) > sinrHysteresisDb) {
            if (ueRrc->GetState() == LteUeRrc::CONNECTED_NORMALLY && ueRrc->GetRnti() != 0) {
                std::cout << "[INFO] Triggering SINR-based handover for UE NodeId: " << ueNodeId
                          << "\n  From eNB NodeId: " << servingEnbNodeId << " (CellId: " << servingEnb->GetCellId()
                          << ", SINR: " << servingSinrDb << " dB)"
                          << "\n  To eNB NodeId: " << bestEnbNodeId << " (CellId: " << bestEnbDev->GetCellId();
                        //   << ", SINR: " << bestSinrDb << " dB)"
                        //   << "\n  Improvement: " << (bestSinrDb - servingSinrDb) << " dB\n";
                lteHelper->HandoverRequest(Seconds(0), ueDevice, servingEnb, bestEnbDev);
                // std::cout << "[DEBUG] SINR-based handover request sent successfully" << std::endl;
            }
        }
    }

    // Schedule the next handover decision
    const double HANDOVER_CHECK_INTERVAL = 0.2;
    Simulator::Schedule(
...
void LteGymEnv::CollectEnvData()
{
    ...
    std::cout << "CollectEnvData() called" << std::endl;
    // Time now = Simulator::Now();
    ...
    for (const auto& [sbsId, model] : m_energyModels) {
        ...
        std::cout << "  [SBS " << sbsId << "] Energy consumption: " << energy << std::endl;
...
bool LteGymEnv::ExecuteActions(Ptr<OpenGymDataContainer> action)
{
    ...
    for (const auto& [sbsId, model] : m_energyModels) {
        ...
        double txPowerDbm = (txPowerW > 0) ? txPowerW : -100;
...
int main(int argc, char *argv[])
{
    ...
    double simulationTime = 60; // seconds (should be double for step arithmetic)
    double stepTime = 0.2; // seconds
    ...
    for (uint32_t i = 0; i < ueNodes.GetN(); ++i) {
        ...
        if (i == 0) {
            // IMSI = 1 (first UE)
            mob->SetPosition(Vector(250.0, 750.0, 0.0));
        } else if (i == 1) {
            // IMSI = 2 (second UE)
            mob->SetPosition(Vector(400.0, 400.0, 0.0));
        }
    ...
    for (uint32_t i = 0; i < macroEnbLteDevs.GetN (); ++i)
    {
        ...
        enbPhy->SetTxPower (45.0);   // value is in dBm
    }
    InitializeSbsTxPower ();
    ...
    Simulator::Schedule(
        Seconds(0.2),
        &CustomSinrHandover,
        lteHelper, ueNodes, enbDevs, ueLteDevs, 3.5, 2.0 
    );

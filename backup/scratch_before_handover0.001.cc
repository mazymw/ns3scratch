...
std::map<uint32_t, std::vector<double>> sbsSinrSamples;
...
void SampleSbsSinr(NetDeviceContainer& enbDevs, NetDeviceContainer& ueDevs, double pathLossExponent) {
    double tNow = Simulator::Now().GetSeconds();
    std::cout << "[SampleSbsSinr] Time=" << tNow << "s — Starting SINR sampling...\n";
    ...
    for (const auto& [sbsId, ueList] : sbsToUeMap) {
        ...
        if (!sbsNode) {
            std::cout << "  [SBS " << sbsId << "] Node not found. Skipping.\n";
            continue;
        }

        Ptr<LteEnbNetDevice> enbDev = sbsNode->GetDevice(0)->GetObject<LteEnbNetDevice>();
        double txPowerDbm = enbDev->GetPhy()->GetTxPower();
        std::cout << "  [SBS " << sbsId << "] TX Power: " << txPowerDbm << " dBm | Checking " << ueList.size() << " UEs...\n";
        ...
        std::cout << "  [SBS " << sbsId << "] Avg SINR = " << avg << " dB from " << count << " valid UEs\n";

        sbsSinrSamples[sbsId].push_back(avg);
    }

    // Schedule next sample at 1 ms if < 0.2s
    if (tNow + 0.2 <= 0.2) {
        Simulator::Schedule(Seconds(0.2), &SampleSbsSinr, std::ref(enbDevs), std::ref(ueDevs), pathLossExponent);
    }
}
...
void LteGymEnv::CollectEnvData()
{
    ...
    Simulator::Schedule(Seconds(0.0), &SampleSbsSinr, std::ref(*globalEnbDevs), std::ref(*globalUeDevs), 3.5);
}
...
int main(int argc, char *argv[])
{
    ...
    for (uint32_t i = 0; i < macroEnbLteDevs.GetN (); ++i)
    {
        ...
        enbPhy->SetTxPower (40.0);   // value is in dBm
    }
    InitializeSbsTxPower ();
